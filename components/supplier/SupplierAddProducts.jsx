...
const SupplierAddProduct = () => {
  ...
  const { currentUser, userData } = useAuth(); // ✅ Used for supplierId/name/number
  ...
  const handleLocationChange = (
    ...
    const updatedRanges = [...priceRanges];
    updatedRanges[priceRangeIndex].locations[locationIndex][field] = value;
    setPriceRanges(updatedRanges);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      showNotification("Validation Error", "Please fill all required fields.");
      return;
    }
    ...
    try {
      ...
      const mainImageRef = ref(
        storage,
        `images/${generateUniqueFileName(mainImage)}`
      );
      ...
      for (let index = 0; index < additionalImages.length; index++) {
        ...
        if (file) {
          const imageRef = ref(
            storage,
            `images/${generateUniqueFileName(file, index)}`
          );
      ...
      await addDoc(collection(db, "products"), {
        ...
        createdAt: new Date(),
        supplierId: currentUser?.uid || "N/A",
        supplierName: userData?.name || "N/A",
        supplierNumber: userData?.contact || "N/A",
      });
      ...
      console.error(err);
      showError("Failed to upload product.");
    } finally {
      setLoading(false);
    }
  };
